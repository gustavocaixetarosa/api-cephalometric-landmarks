...
def regression_voting(heatmaps, R):
    ...
    Xmap = torch.round(
        heatmap[:, landmarkNum:landmarkNum*2, :].data * R).long() * w
    Ymap = torch.round(
        heatmap[:, landmarkNum*2:landmarkNum*3, :].data * R).long()
    ...
    for imageId in range(imageNum):
        for landmarkId in range(landmarkNum):
            ...
            topnXoff = Xmap[imageId][landmarkId][indexs[imageId]
                                                 # offset in x direction
                                                 [landmarkId]]
            topnYoff = Ymap[imageId][landmarkId][indexs[imageId]
                                                 # offset in y direction
                                                 [landmarkId]]
            VotePosi = (topnXoff + topnYoff +
                        indexs[imageId][landmarkId]).cpu().numpy().astype("int")
...
def calculate_deviation(coordinates1, lables):
    ...
    tem_dist = torch.sqrt(
        torch.sum(torch.pow(coordinates1_b - lables_b, 2), 2))
    return tem_dist
