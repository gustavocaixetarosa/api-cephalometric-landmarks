...
class dilationInceptionModule(nn.Module):
    def __init__(self, inplanes, planes):
        ...
        self.temConv3 = nn.Sequential(
            nn.Conv2d(inplanes, fnum, kernel_size=(3, 3),
...
class fusionResNet50(nn.Module):
    def __init__(self, model, batchSize, landmarksNum, useGPU, image_scale, R):
        ...
        for i in range(landmarksNum*3):
            ...
            temConv = nn.Conv2d(
                fnum*4, 1, kernel_size=(1, 1), stride=1, padding=0)
    ...
    def getCoordinate(self, outputs1):
        ...
        heatmap_sum = torch.sum(heatmaps.view(
            self.batchSize, self.landmarksNum, -1), dim=2)

        Xmap1 = heatmaps * self.coordinateX
        Ymap1 = heatmaps * self.coordinateY

        Xmean1 = torch.sum(Xmap1.view(
            self.batchSize, self.landmarksNum, -1), dim=2) / heatmap_sum
        Ymean1 = torch.sum(Ymap1.view(
            self.batchSize, self.landmarksNum, -1), dim=2) / heatmap_sum
        ...
        coordinateDev = torch.sum(
            (XDevmap + YDevmap).view(self.batchSize, self.landmarksNum, -1), dim=2) / heatmap_sum
...
class fusionVGG19(nn.Module):
    def __init__(self, model, config):
        ...
        self.coordinateX = torch.ones(
            self.batchSize, self.landmarksNum, self.higth, self.width).cuda(config.use_gpu)
        ...
        self.coordinateX, self.coordinateY = self.coordinateX / \
            (self.higth - 1), self.coordinateY / (self.width - 1)
