...
# ~ self.binary_class_groundTruth = Variable(torch.zeros(imageNum, landmarkNum, h, w).cuda(self.use_gpu))
        self.offsetMapX_groundTruth = Variable(torch.zeros(
            self.imageNum, self.landmarkNum, self.higth, self.width).cuda(self.use_gpu))
        self.offsetMapY_groundTruth = Variable(torch.zeros(
            self.imageNum, self.landmarkNum, self.higth, self.width).cuda(self.use_gpu))
        self.binary_class_groundTruth1 = Variable(torch.zeros(
            self.imageNum, self.landmarkNum, self.higth, self.width).cuda(self.use_gpu))
        self.binary_class_groundTruth2 = Variable(torch.zeros(
            self.imageNum, self.landmarkNum, self.higth, self.width).cuda(self.use_gpu))
        self.offsetMask = Variable(torch.zeros(
            self.imageNum, self.landmarkNum, self.higth, self.width).cuda(self.use_gpu))
        ...
        self.HeatMap = Variable(torch.from_numpy(
            self.HeatMap)).cuda(self.use_gpu).float()
        self.mask = Variable(torch.from_numpy(self.mask)
                             ).cuda(self.use_gpu).float()
        self.offsetMapx = Variable(torch.from_numpy(self.offsetMapx)).cuda(
            self.use_gpu).float() / config.R2
        self.offsetMapy = Variable(torch.from_numpy(self.offsetMapy)).cuda(
            self.use_gpu).float() / config.R2
        ...
        def getOffsetMask(self, h, w, X, Y):
            for imageId in range(self.imageNum):
                for landmarkId in range(self.landmarkNum):
                    self.offsetMask[imageId, landmarkId, :, :] = self.mask[h - X[imageId][landmarkId]: 2 *
                                                                           h - X[imageId][landmarkId], w - Y[imageId][landmarkId]: 2*w - Y[imageId][landmarkId]]
        ...
        def forward(self, featureMaps, landmarks):
            ...
            for imageId in range(self.imageNum):
                for landmarkId in range(self.landmarkNum):
                    ...
                    binary_class_groundTruth[imageId, landmarkId, :, :] = self.HeatMap[h - X[imageId][landmarkId]
                        : 2*h - X[imageId][landmarkId], w - Y[imageId][landmarkId]: 2*w - Y[imageId][landmarkId]]
                    self.offsetMapX_groundTruth[imageId, landmarkId, :, :] = self.offsetMapx[h - X[imageId][landmarkId]
                        : 2*h - X[imageId][landmarkId], w - Y[imageId][landmarkId]: 2*w - Y[imageId][landmarkId]]
                    self.offsetMapY_groundTruth[imageId, landmarkId, :, :] = self.offsetMapy[h - X[imageId][landmarkId]
                        : 2*h - X[imageId][landmarkId], w - Y[imageId][landmarkId]: 2*w - Y[imageId][landmarkId]]
            ...
            temloss = [[2*self.binaryLoss(featureMaps[imageId][landmarkId], binary_class_groundTruth[imageId][landmarkId]),
                       ...
                       for imageId in range(self.imageNum)
                       for landmarkId in range(self.landmarkNum)]
            loss1 = (sum([sum(temloss[ind]) for ind in range(
                self.imageNum * self.landmarkNum)]))/(self.imageNum * self.landmarkNum)
